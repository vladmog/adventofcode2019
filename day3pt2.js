...
const returnPointsBetweenCoords = (coord1, coord2, steps) => {
  let x1 = coord1[0]
  let y1 = coord1[1]
...
const plotCoordsArr = (wire) => {
  ...
  for (i = 0; i < wire.length; i++){
    ...
    let pathBetween = returnPointsBetweenCoords([x1, y1], [x2, y2], steps)

    steps += pathBetween.length
    coords = coords.concat(pathBetween)

    coords.push([x2, y2, steps])
...
// Derives coordinate plot of each wire, along with the amount of steps it took to get to each coordinate
let wire1PlotArr = plotCoordsArr(wire1)
let wire2PlotObj = plotCoordsObj(wire2)


let leastTotalSteps = Infinity


// Find the intersection that is the fewest amount of steps from the intersection
for (let i = 0; i < wire1PlotArr.length; i++){
  let x = wire1PlotArr[i][0]
  let y = wire1PlotArr[i][1]
  let wire1Steps = wire1PlotArr[i][2]
  let coordString = `${x},${y}`

  if (coordString in wire2PlotObj){
    let wire2Steps = wire2PlotObj[coordString]
    let totalSteps = wire1Steps + wire2Steps

  
    if (totalSteps < leastTotalSteps){
      leastTotalSteps = totalSteps
    }
  }
}


console.log("Least steps: ", leastTotalSteps)