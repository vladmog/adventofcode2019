...
// Double-Checker stops the launch sequence. Apparently, you forgot to include additional fuel for the fuel you just added.
...
// should instead be treated as if it requires zero fuel; the remaining mass, if any, is instead handled by wishing really hard, 
...
const calcFuelRecursive = (mass) => 
{
    ...
    if (fuelRequired > 0){
        calcFuelRecursive(fuelRequired)
    } else {
        return
    }
    
    totalFuelReq += fuelRequired
    return totalFuelReq
}
...
{
    totalFuelReq = 0
    return acc + calcFuelRecursive(mass)
}, 0)

console.log(totalFuelRequired) // Answer: 4967616