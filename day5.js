...
// you get the air conditioner working by upgrading your ship computer to support the 
...
// (your puzzle input). The TEST diagnostic program will run on your existing Intcode 
...
// first parameter's mode is in the hundreds digit, the second parameter's mode is in 
// the thousands digit, the third parameter's mode is in the ten-thousands digit, and so 
// on. Any missing modes are 0.

// For example, consider the program 1002,4,3,4,33.

// The first instruction, 1002,4,3,4, is a multiply instruction - the rightmost two digits 
// of the first value, 02, indicate opcode 2, multiplication. Then, going right to left, 
// the parameter modes are 0 (hundreds digit), 1 (thousands digit), and 0 (ten-thousands 
...
// working correctly; check the instructions that were run before the output instruction 
// to see which one failed.

// Finally, the program will output a diagnostic code and immediately halt. This final 
// output isn't an error; an output followed immediately by a halt means the program finished. 
// If all outputs were zero except the diagnostic code, the diagnostic program ran 
// successfully.
